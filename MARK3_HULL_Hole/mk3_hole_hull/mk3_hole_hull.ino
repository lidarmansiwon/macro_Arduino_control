...
void loop() {
  ...
  if (avg3 > 1482 && avg3 < 1528) {
    avg3 = 1500;
  }
  if (avg4 > 1482 && avg4 < 1528) {
    avg4 = 1500;
  }
  int a;
...
// if (Serial.available()) {
    // Initialize an array to hold the received bytes
    char RecieveData[8];  // Adjust the size if necessary
    int length = Serial.readBytesUntil('\n', RecieveData, sizeof(RecieveData) - 1);
    RecieveData[length] = '\0';  // Null-terminate the string

    int value1, value2;
    sscanf(RecieveData, "%d,%d", &value1, &value2);
  ...
  if (Serial.availableForWrite()) {
    DataTOSend = DataToESC + "," + String(Status) + "," + DataFromRC + "," + a;
